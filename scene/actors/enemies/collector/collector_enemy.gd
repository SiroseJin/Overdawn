...
func move(delta):
	...
	if not taking_damage and is_witch_chase and Global.playerAlive and is_instance_valid(Global.PlayerBody):
		...
		velocity.x = signf(global_position.x - Global.PlayerBody.global_position.x) * 60
...
func handle_animation():
	...
	if not dead and not taking_damage and not is_dealing_damage:
		...
		var facing_left = (is_witch_chase and Player and Player.global_position.x < global_position.x) \
...
func handle_death():
	...
	ProgressionManager.notify("enemy_defeated", {"type": "collector"})   # can unlock lore
	AudioManager.play_sfx("collector_death")
	queue_free()
...
func _tick_attack(delta: float) -> void:
	...
	if is_instance_valid(Global.playerHitbox) and $WitchDealDamageArea.overlaps_area(Global.playerHitbox):
		...
		AudioManager.play_sfx("collector_attack")
