...
# ─── Tutorial Prompt (#11) ────────────────────────────────────────────────────────
# The "Take the tutorial?" Yes/No offer, shown as a real UI prompt AFTER the lobby
# NPC's dialogue ends — whether the player read it through OR skipped it (the old
# in-Dialogic choice got bypassed by the skip button). Emits `answered(take)`.
# ──────────────────────────────────────────────────────────────────────────────────

signal answered(take: bool)
...
func _ready() -> void:
	...
	var title := Label.new()
	title.text = "Ambil tutorialnya?" if id else "Take the tutorial?"
...
func _btn(text: String, take: bool) -> Button:
	var b := Button.new()
	b.text = text
